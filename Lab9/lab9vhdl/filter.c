...
volatile uint16* filter_ptr = (uint16*)FILTER_0_BASE;
...
void timer_isr(void *context)
{
	  uint16 sample,filter_out;
        ...
        *PinPtr = TOGGLE;
	    ...
	    {
	    	sample = SdramPtr[SAMPLE_CNT++];

	    	if (pio_flag == 1)                       //only read right sample if stereo mode
	    	{
	    		*filter_ptr = sample;
	    		filter_out = *filter_ptr;
	    		AudioPtr[3] = filter_out;       //in stereo, output to both sides
...
//if the pointer of button 2 is 1 then set the max value of angle else
//set the angle of min angle
